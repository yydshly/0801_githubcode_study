...
## Revision 21: atmosphere / content experience comparison

The user explicitly reopened the archived desktop experience to compare the current atmosphere-first composition with an original-inspired content-first composition. This is a revision-led, desktop-first Spatial Stage change. It reuses the existing eight local scenes, one persistent WebGL canvas, interactions, glass geometry, particles and trajectory safety rules. Acceptance is:

- add a clearly labelled `Atmosphere / Content` foreground switch and bind it to real `?experience=atmosphere|content` URL state without reloading the page;
- preserve the current atmosphere profile as the default baseline and make switching profiles update the existing renderer instead of rebuilding textures, geometry or the canvas;
- keep the same active images, lifecycle progress and deterministic layout while profiles change so the comparison reflects presentation choices rather than random scene differences;
- make the content profile visibly prioritize internal information through earlier enlargement, longer middle-depth dwell, more readable medium/large worlds, brighter image cores, reduced refraction/edge wash and quieter particles;
- retain recognizable glass thickness, Fresnel edges, stable fixed-size particles, four-quadrant distribution, non-overlap, complete edge exit, no camera intersection and no recycle flash in both profiles;
- keep selection, hover labels, detail navigation, composer, Escape and keyboard world shortcuts working in both profiles;
- limit new calibration to desktop; preserve the existing narrow-layout fallback without introducing a separate mobile redesign;
- verify direct URL entry, live switching in both directions, visible content clarity difference, canvas continuity, runtime metrics, browser logs, build, Sites tests and full research-gallery build.

## Revision 22: Content v2 information-first hardening

The user authorized implementation and documentation of the remaining content-mode optimization list. This remains a revision-led, desktop-first Immersive Spatial Stage change. The WebGL scene stays persistent and the readable DOM layer gains a stable content focus. Acceptance is:

- stabilize the content composition around three to five readable medium/large worlds, with one explicit focus world and lower-priority background worlds instead of equal visual weight;
- add a bounded 2.5D proxy-depth parallax inside image cores so subject and background respond differently to view movement, while documenting that this is not a true generated depth map;
- expose the current focus world's title, category, short description and semantic Enter action in DOM without hiding or replacing direct sphere hover/click;
- reduce central glass competition further while preserving a recognizable Fresnel rim, shell thickness and localized highlight;
- separate optical transition speed from trajectory transition speed so experience switching remains smooth and does not visibly jerk or remount the canvas;
- add a real texture-loading state and lazy-load the Three.js stage so readable foreground chrome appears before the heavy WebGL feature chunk;
- preserve fixed-size particles, deterministic image ownership, continuous edge exits, positive sampled sphere spacing, details, composer, Escape, direct URLs and the existing desktop support boundary;
- verify the Content v2 default frame, focus-card journey, both switch directions, loading/fallback semantics, runtime metrics, browser logs, production build, Sites tests and full research-gallery build.

## Revision 23: Content particle visibility calibration

The user accepted the Content composition and requested one bounded particle correction after the live side-panel view showed that the information-first opacity reduction made the star field too dark. This remains a particle-visibility calibration, not a new effect. Acceptance is:

- keep the existing 1,450/340 pooled particle counts, shared `1.18/1.18 CSS px` sizes, circular masking and uniform frustum distribution;
- preserve continuous depth speed, wheel synchronization, far-in/near-out lifecycle and the absence of an optical-axis cluster;
- raise Content-only particle visibility through opacity rather than count, size growth, streaks or center concentration;
- retain sphere imagery priority, positive projected spacing, stable trajectories, glass settings and all foreground interactions;
- record the final parameter and verify the live Content frame, runtime particle state, browser logs, production build and Sites tests.
...
| GitHub Pages base-path repair | Production HTML loads its JavaScript and CSS from the repository project path and renders the archived experience | Published project route at 1280×720 | Generated HTML, HTTP asset status, Actions deployment, DOM/canvas state and browser logs | Stage 9 | pass | Commit `fb543ec` deployed successfully; production renders one 1280×720 canvas with 8 scenes, 4 physical corridors, 4 controls and 1,450/340 particles, motion advances, details open, and logs/overflow remain clean |
| Dual experience switch | One persistent canvas switches between atmosphere-first and content-first profiles with real URL state | 1280×720 default, both URL entries and both switch directions | DOM state, URL, canvas identity, active scenes, runtime profile metrics and logs | Stage 4 | pass | Direct content entry and two-way switching preserve renderer mount id `2`, reach mix endpoints `0/1`, update pressed/URL state, retain interactions and create no overflow |
| Content-first world clarity | Content mode makes internal scenes immediately more readable without losing glass identity or trajectory safety | 1280×720 timed content-mode sequence and selected detail | Equal-time browser observations, readable-world count, projected size, optical profile, spacing and interaction | Stage 2 | pass | Same-lifecycle switch raises readable worlds `4→6`, central readable worlds `1→3` and largest radius `0.239→0.330`; particles stay fixed-size, optics quieten and sampled projected gap remains positive at `0.032` |
| Content v2 composition | Content mode presents 3–5 readable medium/large worlds with one stable focus and quieter background supports | 1280×720 timed content sequence | Screenshot, readable-role metrics, size distribution and projected gap | Stage 2 | pass | Ten timed samples retained 3–5 primary worlds and 1–2 supports with positive projected spacing; semantic primary images and weighted promotion keep the composition readable without trajectory changes |
| Content particle visibility | Sparse stars remain visible behind readable worlds without changing point size, density or distribution | 809×898 Content mode | Live screenshot, opacity/size/lifecycle runtime state, sphere gap and browser logs | Stage 2 | pass | Content opacity scales are `0.72/0.78`; particles remain fixed at `1.18/1.18 CSS px`, continuous-depth-wheel-synced and far-in/near-out, while the sampled sphere gap stays positive at `0.044` |
| Proxy-depth image core | Content image cores show bounded view-dependent subject/background separation without reducing center clarity | 1280×720 pointer drift and close pass | Shader state, screenshot and pointer-state observation | Stage 2 | pass | Content reports `luma-detail-parallax` at strength `0.920`; pointer movement changed the sampled view shift from about `0.0001/0.0012` to `0.0216/0.0095` while Atmosphere keeps the proxy disabled |
| Content focus journey | A semantic DOM focus card identifies one readable world and enters its existing detail panel | Content mode default, focus change and detail open/close | DOM, click, Escape/focus and canvas persistence | Stage 5 | pass | Throttled focus moved across readable worlds, the card crossed to the opposite side when needed, Enter opened the matching Virtual Try-On detail, and Escape restored the card while preserving the canvas |
| Transition and loading hardening | Optical and trajectory transitions settle smoothly; foreground UI remains readable while the lazy WebGL stage and textures load | Direct URLs, two-way switch and cold load | Canvas identity, mix metrics, loading state, entry/dynamic bundle sizes and logs | Stage 8 | pass | One canvas remained mounted through both switch directions; foreground loading progress covers texture startup, and the gallery build splits a 236.75 kB raw / 72.45 kB gzip entry from a 565.17 kB raw / 144.43 kB gzip WebGL chunk |
