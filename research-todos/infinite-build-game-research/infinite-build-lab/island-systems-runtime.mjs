...
const core = window.IslandSystemsCore;
const level = window.IslandSystemsLevel;
const canvas = document.querySelector("#island-systems-scene");
const SAVE_KEY = "research08:island-systems:v2";
const query = new URLSearchParams(window.location.search);
const requestedFixture = query.get("fixture") || "start";
const validFixtures = new Set(["start", "craft-ready", "combat", "build-ready", "complete", "low-health"]);
...
if (query.get("fallback") === "1") throw new Error("WebGL fallback review fixture");
...
const ui = {
  ...
  hpCount: document.querySelector("#hp-count"),
  hpMeter: document.querySelector("#hp-meter"),
  ...
  enemyMeter: document.querySelector("#enemy-meter"),
  woodCount: document.querySelector("#wood-count"),
  stoneCount: document.querySelector("#stone-count"),
  resinCount: document.querySelector("#resin-count"),
  toolSlot: document.querySelector("#tool-slot"),
  ...
  steps: [document.querySelector("#step-gather"), document.querySelector("#step-craft"), document.querySelector("#step-fight"), document.querySelector("#step-loot"), document.querySelector("#step-build")],
  resultKicker: document.querySelector("#result-kicker"),
  resultTitle: document.querySelector("#result-title"),
  resultCopy: document.querySelector("#result-copy"),
  resultTime: document.querySelector("#result-time"),
  resultTool: document.querySelector("#result-tool"),
  resultWorld: document.querySelector("#result-world"),
};
...
const palette = {
  ...
  stonePickup: new THREE.MeshStandardMaterial({ color: 0x9aafb0, roughness: .92 }),
  resin: new THREE.MeshStandardMaterial({ color: 0x67e0ad, emissive: 0x218f69, emissiveIntensity: 1.5, roughness: .38 }),
  enemyBody: new THREE.MeshStandardMaterial({ color: 0x385948, roughness: .92 }),
  enemyBark: new THREE.MeshStandardMaterial({ color: 0x674a33, roughness: .96 }),
  enemyWarning: new THREE.MeshStandardMaterial({ color: 0xe85d4e, emissive: 0x9f241d, emissiveIntensity: 1.4, roughness: .62 }),
};
...
fireGroup.position.set(level.campfire.x, 0, level.campfire.z);
...
level.pickups.forEach((pickup, index) => {
  const group = new THREE.Group();
  group.position.set(pickup.x, .14, pickup.z);
  ...
  if (pickup.item === "wood") {
    ...
    group.add(mesh(new THREE.DodecahedronGeometry(.3, 0), palette.stonePickup, { x: -.12, y: .12, z: 0 }, { sx: 1.05, sy: .7, sz: .9 }));
    group.add(mesh(new THREE.DodecahedronGeometry(.22, 0), palette.rockLight, { x: .22, y: .08, z: .08 }, { sx: .9, sy: .65, sz: 1.1 }));
  }
  const glowColor = pickup.item === "wood" ? 0xe8bd79 : 0xa8cbd0;
  const glow = mesh(new THREE.RingGeometry(.38, .45, 28), new THREE.MeshBasicMaterial({ color: glowColor, transparent: true, opacity: .58, side: THREE.DoubleSide }), { x: 0, y: -.08, z: 0 }, { rx: -Math.PI / 2, cast: false, receive: false });
  group.add(glow);
  pickupVisuals.set(pickup.id, group);
  world.add(group);
});

const resinVisual = new THREE.Group();
resinVisual.add(mesh(new THREE.OctahedronGeometry(.32, 0), palette.resin, { x: 0, y: .42, z: 0 }, { sy: 1.25 }));
resinVisual.add(mesh(new THREE.RingGeometry(.43, .52, 32), new THREE.MeshBasicMaterial({ color: 0x77e8bd, transparent: true, opacity: .72, side: THREE.DoubleSide }), { x: 0, y: .05, z: 0 }, { rx: -Math.PI / 2, cast: false, receive: false }));
resinVisual.visible = false;
world.add(resinVisual);
...
function createPlayer() {
  ...
  axe.position.set(.47, 1.03, .02);
  axe.rotation.z = -.28;
  axe.add(mesh(new THREE.CylinderGeometry(.035, .045, .72, 7), palette.wood, { x: 0, y: 0, z: 0 }, { rz: .06 }));
  axe.add(mesh(new THREE.BoxGeometry(.34, .2, .08), palette.stonePickup, { x: -.12, y: .31, z: 0 }, { rz: -.15 }));
  axe.visible = false;
  group.add(torso, head, hair, leftLeg, rightLeg, leftArm, rightArm, axe);
  group.userData.parts = { leftLeg, rightLeg, leftArm, rightArm, axe };
  return group;
}
...
function createEnemy() {
  const group = new THREE.Group();
  const body = mesh(new THREE.DodecahedronGeometry(.68, 1), palette.enemyBody, { x: 0, y: .88, z: 0 }, { sx: 1.12, sy: .82, sz: .92 });
  const head = mesh(new THREE.DodecahedronGeometry(.42, 1), palette.enemyBark, { x: 0, y: 1.3, z: -.5 }, { sx: 1.1, sy: .86, sz: .95 });
  const legs = [];
  [[-.42,.35,-.28],[.42,.35,-.28],[-.4,.35,.3],[.4,.35,.3]].forEach(([x,y,z], index) => {
    const leg = mesh(new THREE.CapsuleGeometry(.1, .42, 3, 6), palette.enemyBark, { x, y, z }, { rz: index % 2 ? -.18 : .18 });
    legs.push(leg);
    group.add(leg);
  });
  const antlerLeft = mesh(new THREE.CylinderGeometry(.035, .055, .55, 6), palette.woodLight, { x: -.26, y: 1.72, z: -.48 }, { rz: -.52 });
  const antlerRight = antlerLeft.clone(); antlerRight.position.x = .26; antlerRight.rotation.z = .52;
  const eyeMaterial = new THREE.MeshBasicMaterial({ color: 0x8ff0c5 });
  const eyeLeft = mesh(new THREE.SphereGeometry(.045, 8, 6), eyeMaterial, { x: -.16, y: 1.37, z: -.84 }, { cast: false, receive: false });
  const eyeRight = eyeLeft.clone(); eyeRight.position.x = .16;
  group.add(body, head, antlerLeft, antlerRight, eyeLeft, eyeRight);
  group.userData.parts = { body, head, legs };
  return group;
}

const enemyVisual = createEnemy();
world.add(enemyVisual);

const enemyTelegraph = mesh(new THREE.RingGeometry(.85, 1.38, 44), new THREE.MeshBasicMaterial({ color: 0xef6252, transparent: true, opacity: .72, side: THREE.DoubleSide }), { x: level.enemy.x, y: .08, z: level.enemy.z }, { rx: -Math.PI / 2, cast: false, receive: false });
enemyTelegraph.visible = false;
world.add(enemyTelegraph);
...
let cameraShake = 0;
let lastMessage = "";
let lastPlayerHp = state.player.hp;
let lastEventId = state.eventId;
const keys = new Set();
...
function getObjective() {
  const objective = core.getObjective(state, level);
  return [objective.title, objective.copy, objective.step];
}
...
function updateHud() {
  const [title, copy, activeStep] = getObjective();
  ui.objectiveTitle.textContent = title;
  ui.objectiveCopy.textContent = copy;
  ui.woodCount.textContent = String(state.inventory.wood);
  ui.stoneCount.textContent = String(state.inventory.stone);
  ui.resinCount.textContent = String(state.inventory.resin);
  ui.toolName.textContent = state.equipment.tool === "stoneAxe" ? "石斧" : "空";
  ui.toolSlot.classList.toggle("empty", state.equipment.tool !== "stoneAxe");
  ui.hpCount.textContent = `${Math.round(state.player.hp)}%`;
  ui.hpMeter.style.width = `${state.player.hp}%`;
  const tide = core.tideLevelAt(state.time);
  ui.tideTime.textContent = tide >= 1 ? "高潮" : formatTime(core.secondsUntilHighTide(state.time));
  ui.tideMeter.style.width = `${tide * 100}%`;
  const enemyVisible = core.distance(state.player, state.enemy) <= level.enemy.aggroRadius + 1 || state.enemy.hp < level.enemy.maxHp;
  ui.enemyStatus.textContent = state.enemy.defeated ? "已击退" : (enemyVisible ? `${state.enemy.hp}%` : "未发现");
  ui.enemyMeter.style.width = `${state.enemy.defeated ? 0 : state.enemy.hp}%`;
  ui.steps.forEach((step, index) => { step.className = index < activeStep ? "done" : (index === activeStep ? "active" : ""); });
  if (state.campfire.built) ui.steps.forEach((step) => { step.className = "done"; });
  const prompt = core.getContextPrompt(state, level);
  ui.contextPrompt.hidden = !prompt;
  ui.contextPrompt.textContent = prompt;
...
function showResult(success) {
  ...
  if (success) {
    ui.resultKicker.textContent = "SURVIVAL CHAIN COMPLETE";
    ui.resultTitle.textContent = "第一束火照亮了高地";
    ui.resultCopy.textContent = "采集、装备、战斗、掉落和建造不再是五段演示；它们已经共同改变同一份角色与世界状态。";
  } else {
    ui.resultKicker.textContent = "SURVIVAL CHAIN INTERRUPTED";
    ui.resultTitle.textContent = "潮木兽阻断了生存链";
    ui.resultCopy.textContent = "敌人的红色预警会先于伤害出现。重试时保持距离，在它收招后再挥动石斧。";
  }
  ui.resultTime.textContent = formatTime(state.time);
  ui.resultTool.textContent = state.equipment.tool === "stoneAxe" ? "石斧已装备" : "未装备";
  ui.resultWorld.textContent = state.campfire.built ? "篝火已点燃" : "篝火未完成";
}

function performAction() {
  if (!running || paused || state.status !== "playing") return;
  core.interact(state, level);
  saveGame();
  updateHud();
}
...
function performAttack() {
  if (!running || paused || state.status !== "playing") return;
  if (core.beginPlayerAttack(state)) {
    moveTarget = null;
  }
  updateHud();
}
...
function updateMovement(dt) {
  ...
  const actionLocked = state.player.action !== "idle";
  const speed = (sprinting ? 4.45 : 2.85) * (actionLocked ? .18 : 1);
  ...
  if (moved) {
    core.setPlayerFacing(state, dx, dz);
    playerVisual.rotation.y = Math.atan2(dx, dz);
  }
  return moved;
}
...
function updateWorldVisuals(dt, moved = false) {
  const tide = core.tideLevelAt(state.time);
  const night = state.campfire.built ? 1 : core.clamp((state.time - 118) / 62, 0, .84);
  waterUniforms.uTime.value += dt * (reduceMotion ? .12 : 1);
  ...
  routeMarkers.forEach((marker, index) => {
    marker.material.opacity = Math.max(.08, .52 - tide * .44) * (.82 + Math.sin(state.time * 3 + index) * .18);
  });
  tideHalo.material.opacity = state.crafted.stoneAxe ? .18 : .58 + Math.sin(state.time * 3) * .18;
  ...
  flames.visible = state.campfire.built;
  fireLight.intensity = state.campfire.built ? 6.5 + Math.sin(state.time * 8) * .65 : 0;
  flames.children.forEach((flame, index) => {
    const pulse = reduceMotion ? 1 : 1 + Math.sin(state.time * 8 + flame.userData.phase) * .18;
    flame.scale.set(pulse, reduceMotion ? 1 : 1 + Math.sin(state.time * 6 + index) * .25, pulse);
    if (!reduceMotion) flame.position.x += Math.sin(state.time * 4 + index) * dt * .035;
  });
  ship.visible = state.campfire.built;
  shipLight.scale.setScalar(1 + Math.sin(state.time * 5) * .3);

  pickupVisuals.forEach((visual, id) => {
    const collected = state.collectedPickupIds.includes(id);
  ...
  const attackProgress = state.player.action === "startup" ? .55 : (state.player.action === "active" ? -1.15 : (state.player.action === "recovery" ? -.35 : 0));
  parts.rightArm.rotation.x = state.player.action === "idle" ? stride * .7 : attackProgress;
  parts.rightArm.rotation.z = state.player.action === "idle" ? .15 : -.38;
  parts.axe.visible = state.equipment.tool === "stoneAxe";
  parts.axe.rotation.x = state.player.action === "idle" ? 0 : attackProgress * .72;

  enemyVisual.visible = !state.enemy.defeated;
  enemyVisual.position.set(state.enemy.x, 0, state.enemy.z);
  if (!state.enemy.defeated) {
    enemyVisual.rotation.y = Math.atan2(state.player.x - state.enemy.x, state.player.z - state.enemy.z);
    const warning = state.enemy.state === "telegraph" || state.enemy.state === "active";
    const body = enemyVisual.userData.parts.body;
    body.material = warning ? palette.enemyWarning : palette.enemyBody;
    const bob = state.enemy.state === "approach" ? Math.sin(state.time * 8) * .08 : 0;
    enemyVisual.position.y = bob;
    const lunge = state.enemy.state === "active" ? 1.16 : (state.enemy.state === "telegraph" ? .9 : 1);
    enemyVisual.scale.set(1, 1, lunge);
  }
  enemyTelegraph.visible = state.enemy.state === "telegraph" || state.enemy.state === "active";
  enemyTelegraph.position.set(state.enemy.x, .08, state.enemy.z);
  enemyTelegraph.scale.setScalar(state.enemy.state === "active" ? 1.06 : .86 + Math.sin(state.time * 10) * .06);

  const resinAvailable = state.enemy.dropSpawned && !state.collectedPickupIds.includes(level.enemy.drop.id);
  resinVisual.visible = resinAvailable;
  resinVisual.position.set(level.enemy.drop.x, 0, level.enemy.drop.z);
  if (resinAvailable) {
    resinVisual.rotation.y += dt * 1.2;
    resinVisual.children[0].position.y = .42 + Math.sin(state.time * 3.2) * .08;
    resinVisual.children[1].rotation.z += dt * .65;
  }
...
function tick(time) {
  ...
  if (running && !paused && state.status === "playing") {
    ...
    core.advance(state, level, dt);
    if (state.player.hp < lastPlayerHp) cameraShake = .65;
    lastPlayerHp = state.player.hp;
...
function setKey(event, down) {
  ...
  if (["KeyW", "KeyA", "KeyS", "KeyD", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "ShiftLeft", "ShiftRight", "KeyE", "Space", "Escape"].includes(code)) event.preventDefault();
  if (code === "Escape" && down && !event.repeat) {
    ui.pause.click();
    return;
  }
  if (code === "KeyE" || code === "Space") {
    if (down && !event.repeat) {
      if (code === "Space") performAttack(); else performAction();
    }
    return;
  }
...
document.querySelector("#attack-touch").addEventListener("click", performAttack);
...
window.__ISLAND_SLICE__ = {
  ...
  getSnapshot() {
    return {
      ...
      campfireBuilt: state.campfire.built,
