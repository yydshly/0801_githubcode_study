...
const POLAR_NIGHT_GLSL = /* glsl */ `
  ...
  uniform float uProcessStage;

  vec3 renderStarfield(vec3 viewDirection, float timeFlow, float allowTwinkle) {
    ...
    float twinkleFlow = mix(1.0, blinkAnimation, isTwinkling * allowTwinkle);
  ...
  const backdropMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: auroraGlsl + POLAR_NIGHT_GLSL + /* glsl */ `
      void main() {
        ...
        float stage = uProcessStage;
        vec3 colorValue = paintBackdrop(sightVector);
        if (stage >= 1.0) {
          float processBoost = stage < 3.0 ? 2.6 : 1.0;
          colorValue += renderStarfield(
            sightVector,
            iTime,
            stage >= 2.0 ? 1.0 : 0.0
          ) * uStarIntensity * processBoost * starFade;
        }
        if (stage >= 3.0) {
